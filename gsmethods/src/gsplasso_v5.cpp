...
SEXP gsplasso(SEXP R_x, SEXP R_y,
	...
	if(useNbr == 0){
		...
		ymat = reshape(mat(y.memptr(), y.n_elem, 1, false), nr * nc, ns);
	}
	else{
	...
	NumericMatrix Rcpp_gam(R_gam);
	mat gam(Rcpp_gam.begin(), Rcpp_gam.nrow(), Rcpp_gam.ncol(), false);
	NumericMatrix Rcpp_spindx(R_spindx);
	mat spindx(Rcpp_spindx.begin(), Rcpp_spindx.nrow(), Rcpp_spindx.ncol(), false);
